...
import {
  getColorKey,
  getColorName,
  getPrimaryPropertyContext,
  getPrimaryPropertyContextValue,
  getPrimaryPropertyLabel,
  getPrimaryPropertyOptionValue,
  getSecondaryPropertyLabel,
  getSecondaryPropertyDisplayValue,
  getSecondaryPropertyValue,
  formatPropertyContextValue,
  canBuyProduct,
} from "./helpers";

/**
 * Клиентская интерактивная часть карточки товара.
 * Получает уже готовый массив нормализованных продуктов (серверный fetch +
 * normalizeProduct выполнен в page.tsx). Отвечает за выбор цвета/размера,
 * корзину, шеринг, табы описания, маркетплейсы, ProductSlider.
 *
 * Серверный page.tsx рендерит статичный SEO-блок (название, цена, картинки,
 * описание, характеристики) для поисковиков; этот компонент «оживляет» страницу.
 */
export default function SinglePageClient({
  ...
  const [product, setProduct] = useState<any>(() => {
    const selected =
      initialProducts.find((item: any) => Number(item.id) === Number(id)) ||
      initialProducts.find(canBuyProduct) ||
      initialProducts[0] ||
      null;
    return selected;
  });
  const [isSizeBtn, setIsSizeBtn] = useState<any>(
    () =>
      initialProducts.find((item: any) => Number(item.id) === Number(id)) ||
      initialProducts.find(canBuyProduct) ||
      initialProducts[0]
  ...
  const currentProductTypeID = product?.productTypeID;
                  ...
                  {colors.map((color: any) => (
                    <div
                      ...
                      className={`color-block ${
                        getColorKey(product) === color.color ? "activeColor" : ""
                      }`}
                      onClick={() => {
                          ...
                          >
                            ...
                            {contextValue && (
                              <div className="size-description">
                                {formatPropertyContextValue(context, contextValue)}
                              </div>
                            )}
                          </div>
                        </SwiperSlide>
              ...
              {description === "order_conditions" && (
                <>
                  ...
                  <SpecRow
                    label="Предоплата"
                    value={
                      product?.prepayPercent ? `${product.prepayPercent} %` : "-"
                    }
                  />

                  <SpecRow
                    label="Размер предоплаты"
                    value={product?.prepayAmount ? `${product.prepayAmount} ₽` : "-"}
                  />
