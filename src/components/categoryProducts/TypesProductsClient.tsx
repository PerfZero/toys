...
import { useLazyGetProductsByTypeQuery } from "@/store/api/productsApi";
...
/**
 * Клиентский остров страницы типа товара (/type/:id).
 *
 * Получает начальные товары (initialProducts) серверно из page.tsx — это
 * стартовое состояние `products` вместо фетча на маунте. Дальнейшая пагинация
 * («Показать еще») идёт через RTK Query lazy-хук, как и раньше.
 *
 * id приходит из props (серверный page.tsx делает await params),
 * поэтому useParams здесь не нужен.
 */
function TypesProductsClient({ initialProducts, id }: { initialProducts: any[]; id: string }) {
  ...
  // Начальные товары уже получены серверно — пропускаем первый фетч (offset 0).
  const isFirstFetchSkipped = useRef(false);
  ...
  useEffect(() => {
    // Первая страница уже получена серверно (initialProducts) — не фетчим повторно.
    if (offset === 0) {
      if (!isFirstFetchSkipped.current) {
        isFirstFetchSkipped.current = true;
        return;
      }
    }
...
export default TypesProductsClient;
