...
/**
 * Клиентский остров страницы «Новинки» (/new).
 *
 * Получает начальные товары (initialProducts) серверно из page.tsx — это
 * стартовое состояние `newProducts` вместо фетча на маунте. Дальнейшая пагинация
 * («Показать еще») идёт через RTK Query lazy-хук, как и раньше.
 */
function NewsClient({ initialProducts }: { initialProducts: any[] }) {
  const searchQuery = useSelector((state: any) => state.search.searchQuery);

  const [offset, setOffset] = useState(0);
  const [newProducts, setNewProducts] = useState<any[]>(initialProducts);
